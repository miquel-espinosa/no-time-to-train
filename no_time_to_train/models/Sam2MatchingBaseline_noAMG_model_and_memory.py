...
class Sam2MatchingBaselineNoAMG(nn.Module):
    def __init__(
        ...
        # Print model size summary (parameters + buffers) for encoder and SAM2 predictor
        try:
            self._print_model_size_summary()
        except Exception as e:
            print(f"[ModelSize] Failed to compute model size: {e}")
    ...
    def _compute_module_size_bytes(self, module):
    ...
    def _find_predictor_nn_module(self):
        pred = self.predictor
        if isinstance(pred, nn.Module):
            return pred
        for attr in ["model", "sam", "network", "module"]:
            if hasattr(pred, attr):
                m = getattr(pred, attr)
                if isinstance(m, nn.Module):
                    return m
        for v in pred.__dict__.values():
            if isinstance(v, nn.Module):
                return v
        return None

    def _print_model_size_summary(self):
        try:
            encoder_bytes = self._compute_module_size_bytes(self.encoder)
            pred_module = self._find_predictor_nn_module()
            predictor_bytes = self._compute_module_size_bytes(pred_module)
            total_bytes = encoder_bytes + predictor_bytes
            print(f"[ModelSize] Encoder (parameters + buffers): {self._format_bytes(encoder_bytes)}")
            if pred_module is not None:
                print(f"[ModelSize] SAM2 predictor (parameters + buffers): {self._format_bytes(predictor_bytes)}")
            else:
                print(f"[ModelSize] Predictor module not found for size computation.")
            print(f"[ModelSize] Total model size: {self._format_bytes(total_bytes)}")
        except Exception as e:
            print(f"[ModelSize] Error computing model size: {e}")
    ...
    def forward_test(self, input_dicts, with_negative):
        ...
        device = self.predictor.device
        mem_log_prefix = "[InferenceMemory]"
        is_cuda = torch.cuda.is_available() and (getattr(device, "type", str(device)).startswith("cuda"))
        start_alloc = start_reserved = peak_alloc = None
        cpu_start_rss = None
        if is_cuda:
            try:
            ...
            # Fallback to process RSS on CPU if psutil is available
            try:
                import os as _os
                import psutil as _psutil
                cpu_start_rss = _psutil.Process(_os.getpid()).memory_info().rss
            except Exception:
                cpu_start_rss = None
        ...
        # Print memory usage stats (end + peak)
        if is_cuda:
            try:
                torch.cuda.synchronize(device)
            except Exception:
            ...
            try:
                ...
                peak_alloc = None
            try:
                print(f"{mem_log_prefix} CUDA allocated start -> end: {self._format_bytes(start_alloc)} -> {self._format_bytes(end_alloc)}"
                      f"{'' if peak_alloc is None else f' (peak {self._format_bytes(peak_alloc)})'}")
                print(f"{mem_log_prefix} CUDA reserved start -> end: {self._format_bytes(start_reserved)} -> {self._format_bytes(end_reserved)}")
            except Exception as _e:
                print(f"{mem_log_prefix} Failed to print CUDA memory stats: {_e}")
        else:
            if cpu_start_rss is not None:
                try:
                    import os as _os
                    import psutil as _psutil
                    cpu_end_rss = _psutil.Process(_os.getpid()).memory_info().rss
                    delta = max(0, cpu_end_rss - cpu_start_rss)
                    print(f"{mem_log_prefix} CPU RSS start -> end: {self._format_bytes(cpu_start_rss)} -> {self._format_bytes(cpu_end_rss)} (delta {self._format_bytes(delta)})")
                except Exception as _e:
                    print(f"{mem_log_prefix} Failed to print CPU RSS memory stats: {_e}")
            else:
                print(f"{mem_log_prefix} CUDA not available and psutil not installed; memory usage not reported.")
