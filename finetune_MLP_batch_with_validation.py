...
from coco_dataloader import CocoQueryTargetDataset, CocoValidationDataset
...
def get_args_parser():
    ...
    parser.add_argument("--evaluate", type=str, default=None, help="Evaluate mode. Path to the checkpoint to evaluate")
...
def main(args):
    ...
    if args.evaluate is None: # training
        ...
        # Dataloader
        dataset = CocoQueryTargetDataset(root=f'./data/{split}',
        ...
        try:
            ...
            raise ValueError("Model could not be loaded. Check if the checkpoint has a different architecture? (embed vs no embed).")
            
        if split == 'train2017':
            raise ValueError("Train2017 is not a valid split for evaluation")
        
        # Dataloader
        dataset = CocoValidationDataset(root=f'./data/{split}',
                                    root_queries=f'./data/train2017',
                                    json=f'./data/annotations/instances_{split}_tiny_filtered_by_0.6.json',
                                    image_size=predictor.image_size,
                                    json_queries=f'./data/refsam_eval/20shot_99seed_selectrandom_sortbyarea/20shot_99seed_selectrandom_sortbyarea.json')
        loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True, collate_fn=collate_fn)
        predictions = []
        ground_truths = []
    ...
    for epoch in range(args.epochs):
        ...
        for step, (target, query) in enumerate(tqdm(loader)):
            ...
            if args.evaluate is None: # Training
                raw_points, closest_anns, pos_neg = sample_points(dataset.coco, target_img_info, target_anns, target_bad_anns,
                                                 num_points=args.num_points, ratio_pos_neg=args.ratio_pos_neg)
                # Convert points to torch tensor, and expand to correct shape
                ...
                # Sample points from the grid (already normalized to 0-1)
                raw_points = build_all_layer_point_grids(n_per_side=24, n_layers=0, scale_per_layer=1)[0] # Shape (B, 2)
                points = torch.tensor(raw_points, dtype=torch.float32).unsqueeze(1).to(predictor.device) # Shape (B, 1, 2)
                # Convert to binary mask
                closest_anns = [convert_polygon_to_mask(target_anns[0]['segmentation'],
                                                        width=target_img_info['width'],
                                                        height=target_img_info['height'])]
                closest_anns = torch.tensor(closest_anns, dtype=torch.float32).to(predictor.device) # H, W
                closest_anns = closest_anns.expand(points.shape[0], -1, -1) # B, H, W

            # 1 for positive points, 0 for negative points
            labels = torch.ones((points.shape[0], points.shape[1]), dtype=torch.int32).to(predictor.device) # Shape (B, 1)
            ...
            if args.evaluate is None: # Training
                ...
                # Select the best binary mask, based on the highest IoU prediction,
                # And encode into coco prediction format
                best_iou_idx = torch.argmax(my_iou_pred).item()
                best_logits = target_video_res_masks[best_iou_idx].squeeze().cpu().numpy()
                best_binary_mask = target_video_res_masks_binary[best_iou_idx].squeeze().cpu().numpy()
                # Convert to polygon
                polygons = binary_mask_to_polygons(best_binary_mask)
                
                rle = mask_utils.encode(np.asfortranarray(best_binary_mask.astype(np.uint8)))
                rle['counts'] = rle['counts'].decode('utf-8')
                bbox = mask_utils.toBbox(rle).tolist()
                area = float(mask_utils.area(rle))
                
                prediction = {
                    'image_id': int(target_img_info['id']),
                    'category_id': target_anns[0]['category_id'],
                    'bbox': bbox,
                    'segmentation': rle,
                    'score': my_iou_pred[best_iou_idx].item(),
                    'area': area
                }
                predictions.append(prediction)
                ground_truths.append(target_anns[0])
                ...
                if args.debug:
                    ...
                    # 1. Query image with ground truth mask
                    query_img = Image.open(f'./data/train2017/{query_img_info["file_name"]}')
                    query_mask = convert_polygon_to_mask(query_anns[0]['segmentation'],
                                                        width=query_img_info['width'],
                                                        height=query_img_info['height'])
                    
                    target_img = Image.open(f'./data/{split}/{target_img_info["file_name"]}')
                    target_mask = closest_anns[0].cpu().numpy()
                    
                    axs[0, 0].imshow(query_img)
                    axs[0, 0].imshow(query_mask, alpha=0.5, cmap='jet')
                    axs[0, 0].set_title('Query Image with Ground Truth Mask')
                    axs[0, 0].axis('off')
                    
                    # 2. Heatmap of IOU predictions
                    axs[0, 1].imshow(target_img)
                    scaled_points = torch.tensor(raw_points) * torch.tensor([video_width, video_height]) # Scale from 0-1 to video res
                    scaled_points = scaled_points.cpu().numpy()
                    # add colorbar
                    cbar = axs[0, 1].scatter(scaled_points[:, 0], scaled_points[:, 1], c=my_iou_pred.detach().cpu().numpy(),
                                                cmap='jet', s=100, alpha=0.75)
                    for polygon in polygons:
                        x_points = polygon[0::2]
                        y_points = polygon[1::2]
                        axs[0, 1].plot(x_points, y_points, 'r-', linewidth=2)
                    axs[0, 1].set_title('Heatmap of IOU Predictions')
                    axs[0, 1].set_xlabel('X Coordinate')
                    axs[0, 1].set_ylabel('Y Coordinate')
                    plt.colorbar(cbar, ax=axs[0, 1], fraction=0.046, pad=0.04)
                    
                    axs[1, 0].imshow(target_img)
                    axs[1, 0].imshow(target_mask, alpha=0.5, cmap='jet')
                    axs[1, 0].set_title('Target Image with Ground Truth Mask')
                    axs[1, 0].axis('off')
                    
                    # 4. Logits prediction for best scoring IOU prediction
                    # Plot the polygon on the image
                    axs[1, 1].imshow(target_img)
                    axs[1, 1].imshow(best_logits, cmap='jet', alpha=0.5)
                    axs[1, 1].set_title(f'Logits Prediction (Best IOU: {my_iou_pred[best_iou_idx]:.4f})')
                    axs[1, 1].axis('off')
                    
                    plt.tight_layout()
                    plt.savefig(f"{saved_plots_path}/evaluation_grid_epoch_{epoch}_step_{step}.png", dpi=300, bbox_inches='tight')
                    plt.close()
                    
                    break
                    
        
            if args.evaluate is not None and step % 500 == 0:
                
                # Print final coco metrics using pycocotools
                from pycocotools.coco import COCO
                from pycocotools.cocoeval import COCOeval
                
                coco_gt = COCO(f'./data/annotations/instances_{split}_tiny_filtered_by_0.6.json')
                coco_pred = coco_gt.loadRes(predictions)
                coco_gt = coco_gt.loadRes(ground_truths)
