#!/usr/bin/env python3
"""
Convert VEDAI512 dataset to DIOR format for the pipeline.

VEDAI512 format:
- Vehicules512/: Contains {id}_co.png (color) and {id}_ir.png (infrared) images
- Annotations512/: Contains per-image annotation files {id}.txt
  Annotation format per line: x_center y_center orientation class_id is_visible is_occluded x1 x2 x3 x4 y1 y2 y3 y4
- Annotations512/foldXX.txt: Training image IDs for fold XX
- Annotations512/foldXXtest.txt: Test image IDs for fold XX
...
def main():
    parser = argparse.ArgumentParser(description='Convert VEDAI512 dataset to DIOR format')
    parser.add_argument('--vedai_dir', type=str, 
                        default='/home/s2254242/projects/no-time-to-train/data/VEDAI512',
                        help='Path to VEDAI512 dataset')
    ...
    # Define paths
    images_dir = original_files / 'Vehicules512'
    annotations_dir = original_files / 'Annotations512'
    ...
    print(f"VEDAI512 dataset: {original_files}")
