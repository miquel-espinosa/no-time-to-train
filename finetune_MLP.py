...
def sample_points(coco, target_img_info, target_anns, target_bad_anns, num_points=32, ratio_pos_neg=0.5):
    ...
    return points, closest_anns, pos_neg
...
class MyModel(nn.Module):
    def __init__(self, transformer_dim, iou_head_hidden_dim, num_mask_tokens, iou_head_depth, iou_prediction_use_sigmoid):
...
def plot_query_target_masks(query_img_info, target_img_info, query_video_res_masks,
                            ...
                            my_iou_pred, iou_gt, target_video_res_masks_binary, path_and_filename, args):
    ...
    target_pred_mask = target_video_res_masks[0].detach().squeeze().cpu().numpy()
    ...
    axs[0, 0].set_title("Query Image with Predicted Mask")
    axs[0, 0].axis('off')

    # Plot target image and predicted mask
    axs[0, 1].imshow(pil_target_img)
    axs[0, 1].imshow(target_pred_mask, cmap='jet', alpha=0.5)
    axs[0, 1].plot(raw_point[0, 0, 0], raw_point[0, 0, 1], 'y*', markersize=20)
    axs[0, 1].set_title("Target Image with Predicted Mask")
    axs[0, 1].axis('off')

    # Plot target image and binary predicted mask
    axs[1, 0].imshow(pil_target_img)
    axs[1, 0].imshow(gt_target_mask, cmap='jet', alpha=0.5)
    axs[1, 0].set_title("Target Image with Ground Truth Mask")
    axs[1, 0].axis('off')

    # Plot target image with point
    axs[1, 1].imshow(pil_target_img)
    axs[1, 1].imshow(target_video_res_masks_binary, cmap='jet', alpha=0.5)
    axs[1, 1].plot(raw_point[0, 0, 0], raw_point[0, 0, 1], 'y*', markersize=20)
    axs[1, 1].set_title(f"Target Image with Point (IOU predicted: {my_iou_pred.detach().squeeze().cpu().numpy():.2f}, IOU ground truth: {iou_gt.squeeze():.2f})")
    axs[1, 1].axis('off')
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for step, (target, query) in tqdm(enumerate(train_loader)):
            ...
            points, closest_anns, pos_neg = sample_points(train_dataset.coco, target_img_info, target_anns, target_bad_anns,
                                                 num_points=args.num_points, ratio_pos_neg=args.ratio_pos_neg)
            
            # my_point = points[0]
            # my_label = 0
            # my_point = torch.tensor([my_point], dtype=torch.float32).unsqueeze(0) # It should be a tensor of shape (1, 1, 2). add batch dimension
            # my_label = torch.tensor([my_label], dtype=torch.int32).unsqueeze(0) # It should be a tensor of shape (1, 1). add batch dimension

            for n_point, (raw_point, closest_ann) in enumerate(zip(points, closest_anns)):
                raw_label = 1 # 1 for positive points, 0 for negative points
                raw_point = np.flip(raw_point, axis=0) # Flip the point to match the format of the model
                ...
                ( 
                    ...
                    target_object_score_logits # output of the occlusion MLP
                        ...
                        #
                        merge_sparse_with_my_token=mymodel.iou_embed.weight.unsqueeze(0),
                ...
                iou = compute_iou(pred_mask=target_video_res_masks_binary, gt_mask=closest_ann)
                
                iou_loss = torch.abs(my_iou_pred - iou)
                # print("iou: ", iou)
                # print("my_iou_pred: ", my_iou_pred)
                ...
                if step % 500 == 0 and n_point % 8 == 0:
                    path_and_filename = f"{saved_plots_path}/pred_masks_epoch_{epoch}_step_{step}_point_{n_point}.png"
                    plot_query_target_masks(query_img_info, target_img_info, query_video_res_masks, closest_ann, target_video_res_masks,
                                            raw_point, my_iou_pred, iou, target_video_res_masks_binary, path_and_filename, args)

                if step % 50 == 0 and n_point == 0:
                    # print("iou_loss: ", iou_loss.item())

                    if args.wandb is not None:
                        wandb.log({"iou_loss": iou_loss.item(), "iou_pred": my_iou_pred.item(), "iou_gt": iou.item()})
                        # wandb.log({"masks_plot": wandb.Image(f"./plots/masks_plot_{step}.png")})
                ...
                if args.debug:
                    print("============================================================")
                    print("================ SAM 2 ORIGINAL OUTPUTS ====================")
                    print("============================================================")
                    
                    inference_state = predictor.init_state(video_path=None, img_paths=[
                        f'./data/{split}/{query_img_info["file_name"]}',
                        f'./data/{split}/{target_img_info["file_name"]}'
                    ])        
                    ...
                    for frame_idx, object_ids, masks in predictor.propagate_in_video(inference_state):
                        intermediate_logits.append(masks[0].squeeze().cpu().numpy())
                        print(" | frame_idx: ", frame_idx, "logits sum: ", masks[0].squeeze().cpu().numpy().sum())
                    ...
                    print("=====")
                    print("Check query mask after propagation. It doesn't affect previous masks")
                    frame_idx, masks_q = list(video_segments.items())[0] # Select the first frame
                    print(masks_q[0].squeeze().sum(), (mask2 > 0.0).sum())
                    print(masks_q[0].squeeze().sum() == (mask2 > 0.0).sum())
                    print("=====")
                    
                    print("IMPORTANT CHECK here: target mask logits")
                    print(out_mask_logits2[0].squeeze().cpu().numpy().sum())
                    print(intermediate_logits[-1].sum())
                    print(target_video_res_masks[0].squeeze().sum())
                    
                    # CHECK 2: Mask predicted is the same in both cases
                    frame_idx, masks_t = list(video_segments.items())[-1] # Select the last frame
                    ...
                    print("\033[92m" + "CHECK 2: TARGET MASK predicted is the same in both cases" + "\033[0m")
                    print(masks_t[0].squeeze().shape, target_video_res_masks_binary.squeeze().shape)
                    print(masks_t[0].squeeze().sum() == target_video_res_masks_binary.squeeze().sum())
                    print(masks_t[0].squeeze().sum(), target_video_res_masks_binary.squeeze().sum())
                    if masks_t[0].squeeze().sum() != 0:
                        print("HEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEeEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEeEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEYYYYYYY")
                    # print(out_mask_logits2[0].squeeze().cpu().numpy().sum(), target_video_res_masks.squeeze().sum().item())
                    # print(out_mask_logits2[0].squeeze().cpu().numpy().sum() == target_video_res_masks.squeeze().sum().item())
                    print("---")
                    ...
                    # Query masks
                    query_masks = [
                        (query_real_gt_mask, "Query Ground Truth"),
                        (my_query_pred_logits, "Query Our Prediction"),
                        (sam_query_pred_logits, "Query SAM Prediction")
                    ]

                    for i, (mask, title) in enumerate(query_masks):
                        axs[0, i].imshow(pil_query_img)
                        im = axs[0, i].imshow(mask, cmap='jet', alpha=0.7)
                        binary_mask = (mask > 0.0).astype(np.float32)
                        # Do not plot the values for binary_mask==0
                        binary_mask[binary_mask == 0] = np.nan
                        axs[0, i].contour(binary_mask, colors='white', linewidths=1)
                    ...
                    # Target masks
                    target_masks = [
                        (target_real_gt_mask, "Target Ground Truth"),
                        (my_target_pred_logits, "Target Our Prediction"),
                        (sam_target_pred_logits, "Target SAM Prediction")
                    ]

                    for i, (mask, title) in enumerate(target_masks):
                        axs[1, i].imshow(pil_target_img)
                        im = axs[1, i].imshow(mask, cmap='jet', alpha=0.5)
                        # binary_mask = (mask > 0.0).astype(np.float32)
                        # # Do not plot the values for binary_mask==0
                        # binary_mask[binary_mask == 0] = np.nan
                        # axs[1, i].contour(binary_mask, colors='black', linewidths=10)
                        axs[1, i].set_title(title)
                        axs[1, i].axis('off')
                        plt.colorbar(im, ax=axs[1, i], fraction=0.046, pad=0.04)
                        
                    # Target masks binary predictions
                    target_masks_binary = [
                        (target_real_gt_mask, "Target Ground Truth"),
                        ((my_target_pred_logits > 0.0).astype(np.float32), "Target Our Binary Prediction"),
                        ((sam_target_pred_logits > 0.0).astype(np.float32), "Target SAM Binary Prediction")
                    ]
                        
                    for i, (mask, title) in enumerate(target_masks_binary):
                        axs[2, i].imshow(pil_target_img)
                        im = axs[2, i].imshow(mask, cmap='jet', alpha=0.5)
                        axs[2, i].set_title(title)
                        axs[2, i].axis('off')
                        plt.colorbar(im, ax=axs[2, i], fraction=0.046, pad=0.04)

                    # Plot the point on target images
                    for i in range(3):
                        axs[1, i].plot(raw_point.squeeze().numpy()[0], raw_point.squeeze().numpy()[1], 'y*', markersize=15)
                        axs[2, i].plot(raw_point.squeeze().numpy()[0], raw_point.squeeze().numpy()[1], 'y*', markersize=15)

                    plt.tight_layout()
                    plt.savefig(f"{saved_plots_path}/plot_{step}.png", dpi=300, bbox_inches='tight')
                    plt.close()
                    
                    exit()
        
        if args.wandb is not None and epoch % 10 == 0:
